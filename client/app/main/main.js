...
.controller('MainController', ['$scope', '$state', 'Search',
  function($scope, $state, Search) {
    ...
    $scope.conductSearch = function(search, choice) {
      ...
      //This is a hack, we need to change
      Search.type = choice ? choice.value : "movies";

      $scope.input = '';
      // All events are handled in one page, type is only utilized in order to
      // conduct the proper query to the api
      if (Search.type === 'music' || Search.type === 'sports') {
